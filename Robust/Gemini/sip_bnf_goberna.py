...
class LBP:
    def __init__(self):
        ...
        self.x1 = self.model.add_variable(lb=-1, ub=1000)
    ...
    def add_discretization(self, y_value):
        self.model.add_linear_constraint(-y_value * self.x1 - (1 -y_value) * self.x2 <= 0)

class LLP:
    def __init__(self):
        self.model = xpress.Model()
        self.model.set_model_attribute(poi.ModelAttribute.Silent, True)
        self.y = self.model.add_variable(lb=0.0, ub=1)
        self.current_x = []
    ...
    def update_with_lbp_solution(self, x_val):
        ...
        self.model.set_objective(-(-self.y * x1 - (1-self.y) * x2), poi.ObjectiveSense.Minimize)
