...
def get_ptdf(net: pn.Network) -> float:
    sa = psa.create_dc_analysis()
    sa.add_branch_flow_factor_matrix(["L12"], ["G2"], "PTDF")
    res = sa.run(net, parameters=plf.Parameters(distributed_slack=False))
    val = float(res.get_sensitivity_matrix("PTDF").loc["G2","L12"])
    print(f"PTDF(L12, G2) = {val:.6f}  (analytical = -1.0)\n")
    return val
...
def solve_ptdf(ptdf_val: float) -> dict:
    """
    ...
    m.add_linear_constraint(P12 - ptdf_val*Pg2, poi.Eq, ptdf_val*(-PD_B2), name="ptdf")
    m.add_linear_constraint(Pg2 + P12,           poi.Eq, PD_B2,             name="bal_B2")
